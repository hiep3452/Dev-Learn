        ...
        try {
            await sendEmail({
                ...
                html:
                    ...
                    otp.toString() +
                    "</strong></p> <p>Vui lòng nhập mã OTP này vào DevLearn Blog để xác nhận tài khoản của bạn.</p> <p>Mã OTP này chỉ có giá trị trong 1 phút.</p>  <p>Trân trọng,</p>",
            });
    ...
    try {
        ...
        if (validUser.isBan) {
            return next(
                errorHandler(
                    401,
                    "Your account has been locked due to violating the blog's terms"
                )
            );
        }
